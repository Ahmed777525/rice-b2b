    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-9 col-lg-10 p-4">
                <h2 class="mb-4">إعدادات الدفع</h2>
                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.settings.payment') }}" method="POST">
                            ...
                            <div class="mb-3 form-check">
                                <input type="checkbox" name="visa_enabled" class="form-check-input" id="visa" {{ setting()->get('visa_enabled') ? 'checked' : '' }}>
                                <label class="form-check-label" for="visa">Visa</label>
                            </div>
                            <div class="mb-3 form-check">
                                <input type="checkbox" name="mada_enabled" class="form-check-input" id="mada" {{ setting()->get('mada_enabled') ? 'checked' : '' }}>
                                <label class="form-check-label" for="mada">Mada</label>
                            </div>
                            <div class="mb-3 form-check">
                                <input type="checkbox" name="paypal_enabled" class="form-check-input" id="paypal" {{ setting()->get('paypal_enabled') ? 'checked' : '' }}>
                                <label class="form-check-label" for="paypal">PayPal</label>
                            </div>
                            
                            <h5 class="mb-3 mt-4">وضع الاختبار</h5>
                            <div class="mb-3 form-check">
                                <input type="checkbox" name="sandbox_mode" class="form-check-input" id="sandbox" {{ setting()->get('sandbox_mode') ? 'checked' : '' }}>
                                <label class="form-check-label" for="sandbox">تفعيل وضع الاختبار (Sandbox)</label>
                            </div>
