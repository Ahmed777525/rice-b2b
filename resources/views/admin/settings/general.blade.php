                    ...
                    <div class="card-body">
                        <form action="{{ route('admin.settings.general') }}" method="POST">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="app_name" class="form-control" value="{{ setting()->get('app_name', 'أرز性质的') }}">
                            </div>
                            <div class="mb-3">
                                <label>وصف التطبيق</label>
                                <textarea name="app_description" class="form-control" rows="3">{{ setting()->get('app_description', '') }}</textarea>
                            </div>
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    <label>بريد التواصل</label>
                                    <input type="email" name="contact_email" class="form-control" value="{{ setting()->get('contact_email', '') }}">
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label>هاتف التواصل</label>
                                    <input type="text" name="contact_phone" class="form-control" value="{{ setting()->get('contact_phone', '') }}">
                                </div>
                            </div>
                            <div class="mb-3">
                                <label>العنوان</label>
                                <textarea name="address" class="form-control" rows="2">{{ setting()->get('address', '') }}</textarea>
                            </div>
